...
// Define the Accesories schema directly in the migration script to avoid circular dependencies
  ...
  try {
    ...
    for (const product of productsWithDistanceCode) {
      await Accessories.updateOne(
        { _id: product._id },
        { $set: { distance_code: product.code } },
      );
    }

    console.log(
      `Updated code field for ${productsWithDistanceCode.length} accessories`,
    );
    ...
    const productsWithSeries = await Accessories.find({
      system: { $exists: true },
    });
    ...
    for (const product of productsWithSeries) {
      await Accessories.updateOne(
        { _id: product._id },
        { $set: { product_group: product.system } },
      );
    }
    ...
    const productsWithStringName = await Accessories.find({
      name: { $type: 'string' },
    });
    ...
    for (const product of productsWithStringName) {
      ...
      await Accessories.updateOne({ _id: product._id }, { $set: nameUpdate });
    }
    ...
    const productsWithNumberPrice = await Accessories.find({
    ...
    const result = await Accessories.updateMany(
